...
namespace PC {
	static GLenum ShaderDataTypeToOpenGLBaseType(ShaderDataType shaderDataType){
		...
		PC_CORE_ERROR("Unknown ShaderDataType {0}.", (int)shaderDataType);
		return 0;
	}
    ...
    VertexArray::VertexArray() {
        ...
        PC_CORE_TRACE("Create Vertex Array '{0}'", m_RendererID);
    }

    VertexArray::~VertexArray() {
        // PC_PROFILE_FUNCTION();
        PC_CORE_TRACE("Delete Vertex Array '{0}'", m_RendererID);
    ...
    void VertexArray::AddVertexBuffer(const Ref<VertexBuffer> &vertexBuffer) {
        ...
        if(!vertexBuffer->GetLayout().GetElements().size()){
            PC_CORE_ERROR("The vertex buffer doesn't have a layout.");
        }
